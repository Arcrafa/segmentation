...
def mobilenetv2_graph(inputs, architecture, alpha = 1.0, train_bn = False):
    ...
    C1 = x = _inverted_residual_block(x, 16,  (3, 3), t=1, strides=1, n=1, alpha=alpha, block_id=1, train_bn=train_bn)	# Input Res: 1/2
    C2 = x = _inverted_residual_block(x, 24,  (3, 3), t=6, strides=2, n=2, alpha=alpha, block_id=2, train_bn=train_bn)	# Input Res: 1/2
    C3 = x = _inverted_residual_block(x, 32,  (3, 3), t=6, strides=2, n=3, alpha=alpha, block_id=4, train_bn=train_bn)	# Input Res: 1/4
    x      = _inverted_residual_block(x, 64,  (3, 3), t=6, strides=2, n=4, alpha=alpha, block_id=7, train_bn=train_bn)	# Input Res: 1/8
    C4 = x = _inverted_residual_block(x, 96,  (3, 3), t=6, strides=1, n=3, alpha=alpha, block_id=11, train_bn=train_bn)	# Input Res: 1/8
    x      = _inverted_residual_block(x, 160, (3, 3), t=6, strides=2, n=3, alpha=alpha, block_id=14, train_bn=train_bn)	# Input Res: 1/16
    C5 = x = _inverted_residual_block(x, 320, (3, 3), t=6, strides=1, n=1, alpha=alpha, block_id=17, train_bn=train_bn)	# Input Res: 1/32
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    pre_nms_class_scores = tf.gather(probs, keep)
    ...
    class_scores_keep = tf.gather(pre_nms_scores, nms_keep)
    num_keep = tf.minimum(tf.shape(class_scores_keep)[0], roi_count)
    top_ids = tf.nn.top_k(class_scores_keep, k=num_keep, sorted=True)[1]
    nms_keep = tf.gather(nms_keep, top_ids)

    # Arrange output as [N, (y1, x1, y2, x2, class_id, score, ... other scores ...)]
    # Coordinates are normalized.
    detections = tf.concat([
        tf.gather(pre_nms_rois, nms_keep),
        tf.to_float(tf.gather(pre_nms_class_ids, nms_keep))[..., tf.newaxis],
        tf.gather(pre_nms_scores, nms_keep)[..., tf.newaxis],
        tf.gather(pre_nms_class_scores, nms_keep),
        ], axis=1)
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    if augment and config.PIXEL_SCALING:
        factor = np.random.normal(1, config.PIXEL_SCALING)
        image[:,:,0] *= factor
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if callable(config.BACKBONE):
            ...
            _, C2, C3, C4, C5 = mobilenetv1_graph(input_image, config.BACKBONE, alpha=config.ALPHA, train_bn=config.TRAIN_BN)
        elif config.BACKBONE in ["mobilenetv2"]:
            _, C2, C3, C4, C5 = mobilenetv2_graph(input_image, config.BACKBONE, alpha=config.ALPHA, train_bn=config.TRAIN_BN)
        else:
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        zero_ix = np.where(detections[:, 4] == -1)[0]
        ...
        for i in range(N):
            ...
            full_mask = utils.unmold_mask(masks[i], boxes[i], original_image_shape, threshold=self.config.MASK_THRESHOLD)
    ...
    def mi_unmold_detections(self, detections, mrcnn_mask, original_image_shape,
                          image_shape, window):
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.
        ...
        class_ids = detections[:N, 4].astype(np.int32)
        scores = detections[:N, 5]
        scores_softmax = detections[:N, 6:13]
        masks = mrcnn_mask[np.arange(N), :, :, class_ids]
        ...
        if exclude_ix.shape[0] > 0:
            ...
            class_ids = np.delete(class_ids, exclude_ix, axis=0)
            scores = np.delete(scores, exclude_ix, axis=0)
            scores_softmax= np.delete(scores_softmax, exclude_ix, axis=0)
            masks = np.delete(masks, exclude_ix, axis=0)
            N = class_ids.shape[0]
        ...
        return boxes, class_ids, scores, scores_softmax, full_masks

    def mi_detect(self, images, verbose=0):
        """Runs the detection pipeline.
        ...
        results = []
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_scores_softmax, final_masks =\
                self.mi_unmold_detections(detections[i], mrcnn_mask[i],
                                       image.shape, molded_images[i].shape,
                                       windows[i])
            ...
            results.append({
                ...
                "class_ids": final_class_ids,
                "scores": final_scores,
                "scores_softmax":final_scores_softmax,
                "masks": final_masks,
            })
        return results
